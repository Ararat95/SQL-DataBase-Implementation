...
void User::insert(const std::string& src) {
    ...
    if (check_user_input_data(parsed)) {
        ...
        for (auto it = parsed.begin(); it < parsed.end(); ++it) {
            // if (keys[*it] == Keyword::EMAIL && check_email(*(it + 1))) {
            //         throw std::invalid_argument ("This email is already exist\n");
            // }
            if (static_cast<bool>(keys[*it])) {
                ...
                tmp->email = (keys[*it] == Keyword::EMAIL) ? *(it + 1) : tmp->email;//check TODO
                tmp->mobile = (keys[*it] == Keyword::MOBILE) ? *(it + 1) : tmp->mobile;//check TODO
                tmp->password = (keys[*it] == Keyword::PASSWORD) ? *(it + 1) : tmp->password;
                ++it;
            }
        // std::cout << *it << '|' << std::endl;        
        }
        //std::cout << tmp->name << '|' << tmp->surname << '|' << tmp->age << '|' << tmp->mobile << '|' << tmp->email << std::endl;
        //delete tmp;
...
bool User::check_email(const std::string& email) {
    for (auto& it : users) {
        if (it.second->email == email) {
            return false;
            // throw std::invalid_argument ("sorry this Email is already exist\n");
        }
    }
    return true;
}
                ...
                if (users[id_to_delete]) {
                    ...
                    switch (com) {
                        case 'y':   users.erase(id_to_delete);
                                    // deleted_ID.push_back(id_to_delete);
                                    update_flag = true;
                                    break;
...
std::vector<std::size_t> User::select(const std::string& src) {
    ...
    // std::copy(selected_data.begin(), selected_data.end(), std::ostream_iterator<std::string>(std::cout, "|"));
    if (found_by_data.empty()) {
        std::cout << "Sorry there are not users found, please try again" << std::endl;
    }
    // else {
    //     std::cout << "Found users below" << std::endl;
    //     for (auto& it : found_by_data) {
    //         print_found_data(it);
    //     }
    // }
    }
